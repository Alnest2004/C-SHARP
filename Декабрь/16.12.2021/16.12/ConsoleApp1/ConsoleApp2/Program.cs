...
namespace ConsoleApp2
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            if (((Math.Sqrt(Math.Pow(x, 2) + Math.Pow(y, 2))) > 3) &&(Math.Sqrt(Math.Pow(x, 2) + Math.Pow(y, 2))) < 7)
            {
                Console.WriteLine("Точка попала в заштрихованную область");
            }
            else if (((Math.Sqrt(Math.Pow(x, 2) + Math.Pow(y, 2))) == 3) || (Math.Sqrt(Math.Pow(x, 2) + Math.Pow(y, 2))) == 7)
            {
